...
export function computeStandings(teams: Team[], rounds: Round[]): TeamStanding[] {
  const standings: TeamStanding[] = teams.map((_, i) => ({
    teamIndex: i,
    wins: 0,
    losses: 0,
    pointsFor: 0,
  ...
  const h2h: Map<number, Set<number>> = new Map();
